...
const App = () => {
    ...
    const [input, setInput] = useState({ day: 'Every day', eventName: '', timesPerDay: 1, timeOfDay: 1 });
    ...
    const handleAddTask = () => {
        daysOfWeek.forEach(day => {
            if (input.day === 'Every day' || 
                ...
                input.day === day) {
                    const newTask = new Task(input.eventName, input.timesPerDay, input.timeOfDay);
                    taskLists[day].addTask(newTask);
                }
        });
        setTaskLists({ ...taskLists });
        setInput({ day: 'Every day', eventName: '', timesPerDay: 1, timeOfDay: ''});
    };
    ...
    return (
        <div>
            ...
            <div>
                ...
                <input type="time" value={input.timeOfDay || ""} 
                    onChange={e => setInput({ ...input, timeOfDay: parseInt(e.target.value) })} />
                ...
                {daysOfWeek.map(day => (
                    <div key={day}>
                        ...
                        {taskLists[day].getTasks().map((task, idx) => (
                            <p key={idx}>Event: {task.eventName}, Times per day: {task.timesPerDay}, Time of day: {task.timeOfDay}</p>
                        ))}
                    </div>
                ))}
            </div>
